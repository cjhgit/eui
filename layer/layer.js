...
;(function (window, $) {
    ...
    var eui = {
        ...
        config: function (options, fn) {
            var item = 0;
            options = options || {};
            eui.cache = ready.config = $.extend(ready.config, options);
            eui.path = ready.config.path || eui.path;
            typeof options.extend === 'string' && (options.extend = [options.extend]);
            eui.use('skin/layer.css', (options.extend && options.extend.length > 0) ? (function loop() {
                var ext = options.extend;
                eui.use(ext[ext[item] ? item : item - 1], item < ext.length ? function () {
                    ++item;
                    return loop;
                }() : fn);
            }()) : fn);
        ...
        use: function (module, fn, readyMethod) {
            ...
            node[iscss ? 'href' : 'src'] = /^http:\/\//.test(module) ? module : eui.path + module;
            node.id = id;
            if (!$('#' + id)[0]) {
            ...
            (function poll() {
                ;
                (iscss ? parseInt($('#' + id).css('width')) === 1989 : eui[readyMethod || id]) ? function () {
                    fn && fn();
                    try {
        ...
        ready: function (path, fn) {
            ...
            if (type) fn = path;
            eui.config($.extend(ready.config, function () {
    ...
    Class.pt.creat = function () {
        ...
        switch (config.type) {
            ...
            case 4:
                ...
                config.tipsMore || eui.closeAll('tips');
                break;
        }
    ...
    Class.pt.openLayer = function () {
        ...
        //置顶当前窗口
        eui.zIndex = that.config.zIndex;
        eui.setTop = function (layero) {
            var setZindex = function () {
                eui.zIndex++;
                layero.css('z-index', eui.zIndex + 1);
            };
            eui.zIndex = parseInt(layero[0].style.zIndex);
    ...
    eui.title = function (name, index) {
        var title = $('#' + doms[0] + (index || eui.index)).find(doms[1]);
        title.html(name);
    };
    ...
    var cache = eui.cache || {}, skin = function (type) {
    ...
    //相册层
    eui.photos = function (options, loop, key) {
        var dict = {};
        options = options || {};
        ...
        dict.loadi = eui.load(1, {
            shade: 'shade' in options ? false : 0.9,
            scrollbar: false
        });
        loadImage(data[start].src, function (img) {
            eui.close(dict.loadi);
            dict.index = eui.open($.extend({
                type: 1,
                area: function () {
