...
!function ($) {
    ...
    var Typeahead = function (element, options) {
        ...
        if (!this.source.length) {
            ...
            if (typeof ajax === 'string') {
                this.ajax = $.extend({}, Typeahead.DEFAULTS.ajax, { url: ajax });
            } else {
    ...
    Typeahead.prototype = {
        ...
        render: function (items) {
            ...
            for (var i = 0; i < len; i++) {
                var item = items[i];
                var ii = $(that.options.item).attr('data-value', item[that.options.val]);
                ii.find('a').html(that.highlighter(item[that.options.display], item));
                var ret = ii[0];
                html += ret.outerHTML;
            }
    ...
    $.fn.typeahead = function (option) {
        return this.each(function () {
            var $this = $(this),
                data = $this.attr('data-typeahead'),
                options = typeof option === 'object' && option;

            if (option.reset || !data) {
                $this.attr('data-typeahead', (data = new Typeahead(this, options)));
            }
